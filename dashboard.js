...
- Cliff Probability: ${(currentCliff * 100).toFixed(1)}%`;
        ...
        client.chat.complete({
            ...
            try {
                ...
                agentTerminal.log(`\n>>> STRATEGY DECISION (Lap ${lap}) <<<`);
                agentTerminal.log(`Decision:   ${parsed.decision}`);
                agentTerminal.log(`Confidence: ${(parsed.confidence * 100).toFixed(1)}%`);
                ...
                try {
                	...
                	agentTerminal.log(`\n>>> STRATEGY DECISION (Lap ${lap}) <<<`);
                 agentTerminal.log(`Decision:   ${parsedFallback.decision}`);
                 agentTerminal.log(`Confidence: ${(parsedFallback.confidence * 100).toFixed(1)}%`);
                 agentTerminal.log(`Reasoning:\n${parsedFallback.reasoning}\n`);
                } catch(fallbackErr) {
    ...
    fs.createReadStream(CSV_FILE)
        ...
        .on('end', () => {
            ...
            const interval = setInterval(() => {
                ...
                const lapNumStr = rawData.LapNumber.padStart(2, '0');
                ...
                if (!isNaN(degradationDelta)) {
                    ...
                    // Trigger AI
                    handleAiDecision(currentLap + 1, result.x, result.P, result.cliffProb);
